...
function drawCirclePartition(d) {
  ...
  function myExtendNode() {
    axios.get('http://192.168.191.3:8023/MapDisplay/subGraph?nodeName=' + d.name)
      .then(res=>{
...
function buildGraph() {
    ...
    // console.log(edges);
  if (!bf) {
    if (histCache.length <= 4) {
      histCache.push({nodes: nodes, links: edges});
    } else {
      histCache.shift();
      histCache.push({nodes: nodes, links: edges});
    }
